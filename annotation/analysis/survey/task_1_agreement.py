...
class InflationNarrative(object):
    ...
    def is_annotation_done(self, project_id): # check if annotation is done
        num_unfinished = self.client.projects.get(id=project_id).queue_total
        is_done = num_unfinished == 0
        return is_done, num_unfinished

    def check_annotation_status(self): # print annotation status
    ...
    def setup_directory(self): # set up directory structure
        for folder in ["./logs/", "./export/"]:
            path = Path(folder)
            if not path.exists():  # Check existence
                path.mkdir(parents=True)

    def instantiate(self, pull_from_label_studio=True): # export project to json and create dataframe
        """
        ...
        self.check_annotation_status() # run check annotation status
        response = self.export_project_to_json(project_id=20, write_to_dist=True)
        inner_id = [document["inner_id"] for document in response]
        text = [document["data"]["text"] for document in response]
        if Path("./export/survey_task_1_annotation.csv").exists():
            df = pd.read_csv("./export/survey_task_1_annotation.csv")
        else:
    ...
    def get_project_title_from_id(self, project_id): # get project title from project id
        print(self.client.projects.get(id=project_id).title)
    ...
    def get_task_1_annotation(self, annotator_id):
        ...
        df.to_csv("./export/survey_task_1_annotation.csv", index=False)
        print(f"Updated annotation from user {annotator_id}, result saved in ./export/survey_task_1_annotation.csv")
