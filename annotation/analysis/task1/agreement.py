...
class InflationNarrative(object):
    ...
    def create_training_data_from_annotation(save_to_disk=True):
        ...
        if Path("./export/task_1_train.csv").exists() and Path("./export/task_1_test.csv").exists():
            df_train = pd.read_csv("./export/task_1_train.csv", index_col=False)
            df_test = pd.read_csv("./export/task_1_test.csv", index_col=False)
        else:
