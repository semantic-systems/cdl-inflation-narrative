...
# --- Exclude and reduce labels before IAA computation ---
...
def map_reduce_and_filter_triples_label_form(triples_label_form: list[tuple]):
    """
    Map low-level labels to reduced high-level groups and remove triples
    containing any excluded labels. Returns a list of mapped triples.
    """
    if not triples_label_form:
        return triples_label_form

    # build reverse mapping: low-level -> reduced group
    reverse_map = {v: k for k, values in REDUCED_LABEL_GROUPS.items() for v in values}

    mapped = []
    for subj, relation, obj in triples_label_form:
        # skip triples where subject or object is in excluded labels
        if subj in EXCLUDED_LABELS or obj in EXCLUDED_LABELS:
            continue

        subj_mapped = reverse_map.get(subj, subj)
        obj_mapped = reverse_map.get(obj, obj)
        mapped.append((subj_mapped, relation, obj_mapped))
...
if __name__ == "__main__":
    ...
    for i, result in enumerate(results):
        ...
        # apply reduction mapping and filter excluded labels before storing
        mapped_triples_label_form = map_reduce_and_filter_triples_label_form(triples_label_form)
        task2_annotation_dict["triples_label_form"].append(mapped_triples_label_form)
    ...
    # create features
    # use reduced label groups for high-level mapping
    event_category = REDUCED_LABEL_GROUPS
